...
class Prognose_PrognosisWindow {
	...
	protected function setDefaultValues() {
		...
		$ResultLine = empty($TopResults) ? __('none') : sprintf( __('%s in %s <small>(%s)</small> and %s in %s <small>(%s)</small>'),
				Distance::format($TopResults[0]['distance']), Duration::format($TopResults[0]['s']), (new UTCTime($TopResults[0]['time']))->format('d.m.Y'),
				Distance::format($TopResults[1]['distance']), Duration::format($TopResults[1]['s']), (new UTCTime($TopResults[1]['time']))->format('d.m.Y')
		);
	...
	protected function runCalculations() {
		...
		foreach ($this->Distances as $km) {
			...
			$DateWithLink = Ajax::trainingLink($PB->activityId(), (new UTCTime( $PB->timestamp() ))->format('d.m.Y'), true);
