...
class RunalyzePlugin_WettkampfStat extends PluginStat {
	...
	private function displayAllCompetitions() {
		...
		$wks = Mysql::getInstance()->fetchAsArray('SELECT * FROM `ltb_training` WHERE `typid`='.WK_TYPID.' ORDER BY `time` DESC');
		foreach ($wks as $i => $wk)
	...
	private function displayLastCompetitions() {
		...
		$wks = Mysql::getInstance()->fetchAsArray('SELECT * FROM `ltb_training` WHERE `typid`='.WK_TYPID.' ORDER BY `time` DESC LIMIT '.$this->config['last_wk_num']['var']);
		if (count($wks) > 0) {
			foreach($wks as $i => $wk)
				$this->displayWkTr($wk, $i);
		} else {
	...
	private function displayPersonalBestsTRs() {
		...
		$dists = Mysql::getInstance()->fetchAsArray('SELECT `distanz`, SUM(1) as `wks` FROM `ltb_training` WHERE `typid`='.WK_TYPID.' GROUP BY `distanz`');
		foreach ($dists as $i => $dist) {
			if ($dist['wks'] > 1) {
				$this->distances[] = $dist['distanz'];
		
				$wk = Mysql::getInstance()->fetchSingle('SELECT * FROM `ltb_training` WHERE `typid`='.WK_TYPID.' AND `distanz`='.$dist['distanz'].' ORDER BY `dauer` ASC');
				$this->displayWKTr($wk, $i);
			}
		}
	}
	...
	private function displayPersonalBestYears() {
		...
		$wks = Mysql::getInstance()->fetchAsArray('SELECT YEAR(FROM_UNIXTIME(`time`)) as `y`, `distanz`, `dauer` FROM `ltb_training` WHERE `typid`='.WK_TYPID.' ORDER BY `y` ASC');
		foreach ($wks as $wk) {
			if (!isset($year[$wk['y']])) {
				$year[$wk['y']] = $dists;
				$year['sum'] = 0;
