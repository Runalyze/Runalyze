...
class ConfigTabSports extends ConfigTab {
	...
	private function getCode() {
		...
		foreach ($Sports as $Data) {
			...
			$SportTypeOptions = $this->getSportTypeOptions($id);
	 ...
	 * @return array
	 */
	protected function getEquipmentTypeOptions($sportid) {
		$Options = array(0 => __('none'));
		$Factory = new \Runalyze\Model\Factory(\SessionAccountHandler::getId());

		foreach ($Factory->equipmentTypeForSport($sportid) as $Type) {
	 ...
	 * @return array
	 */
	protected function getSportTypeOptions($sportid) {
		$Options = array(0 => __('none'));
		$Factory = new \Runalyze\Model\Factory(\SessionAccountHandler::getId());
		
		foreach ($Factory->typeForSport($sportid) as $Type) {
			$Options[$Type->id()] = $Type->name();
		}
