...
if ($Year >= START_YEAR && $Year <= date('Y') && START_TIME != time()) {
	...
	for ($d = 1; $d <= $max; $d++) {
		$ATLs[$d-1]   = 100 * array_sum(array_slice($Trimps_raw, CONF_CTL_DAYS + $d - CONF_ATL_DAYS, CONF_ATL_DAYS)) / CONF_ATL_DAYS / MAX_ATL;
		$CTLs[$d-1]   = 100 * array_sum(array_slice($Trimps_raw, CONF_CTL_DAYS + $d - CONF_CTL_DAYS, CONF_CTL_DAYS)) / CONF_CTL_DAYS / MAX_CTL;
