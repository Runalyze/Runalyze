...
foreach ($Sports as $sport) {
	...
	$data = Mysql::getInstance()->fetchAsArray('SELECT SUM(1) as `num`, SUM(`dauer`) as `value`, HOUR(FROM_UNIXTIME(`time`)) as `h` FROM `ltb_training` WHERE `sportid`="'.$sport['id'].'" AND (HOUR(FROM_UNIXTIME(`time`))!=0 OR MINUTE(FROM_UNIXTIME(`time`))!=0) GROUP BY `h` ORDER BY `h` ASC');
	foreach ($data as $dat)
