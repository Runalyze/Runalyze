...
if ($Year >= START_YEAR && $Year <= date('Y') && START_TIME != time()) {
	...
	$Data = Mysql::getInstance()->fetchAsArray('SELECT (`typid` = '.CONF_WK_TYPID.') as `wk`, SUM(`distanz`) as `km`, WEEK(FROM_UNIXTIME(`time`),1) as `w` FROM `'.PREFIX.'training` WHERE `sportid`='.CONF_RUNNINGSPORT.' AND YEAR(FROM_UNIXTIME(`time`))='.$Year.' GROUP BY (`typid` = '.CONF_WK_TYPID.'), WEEK(FROM_UNIXTIME(`time`),1)');
	foreach ($Data as $dat) {
		if ($dat['wk'] == 1)
	...
	if (CONF_RECHENSPIELE) {
		...
		$AvgMonthPace  = Mysql::getInstance()->fetchSingle('SELECT AVG(`dauer`/60/`distanz`) AS `avg` FROM `'.PREFIX.'training` WHERE `time` > '.(time()-30*DAY_IN_S).' AND `sportid`='.CONF_RUNNINGSPORT);
