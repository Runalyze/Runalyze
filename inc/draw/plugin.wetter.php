...
elseif ($Year >= START_YEAR && $Year <= date('Y') && START_TIME != time()) {
	if ($_GET['m'] == 'm') {
		...
		$Data = Mysql::getInstance()->fetchAsArray('SELECT MONTH(FROM_UNIXTIME(`time`)) as `m`, AVG(`temperature`) as `temp` FROM `'.PREFIX.'training` WHERE !ISNULL(`temperature`) AND YEAR(FROM_UNIXTIME(`time`))='.$Year.' GROUP BY `m` ORDER BY `m` ASC');
		foreach ($Data as $dat)
		...
		$Data = Mysql::getInstance()->fetchAsArray('SELECT DAYOFYEAR(FROM_UNIXTIME(`time`)) as `d`, AVG(`temperature`) as `temp` FROM `'.PREFIX.'training` WHERE !ISNULL(`temperature`) AND YEAR(FROM_UNIXTIME(`time`))='.$Year.' GROUP BY `d` ORDER BY `d` ASC');
		foreach ($Data as $dat)
