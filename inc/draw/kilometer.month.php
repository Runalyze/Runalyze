...
if ($Year >= START_YEAR && $Year <= date('Y')) {
	...
	$Data = Mysql::getInstance()->fetchAsArray('SELECT (`typid` = '.WK_TYPID.') as `wk`, SUM(`distanz`) as `km`, MONTH(FROM_UNIXTIME(`time`)) as `m` FROM `ltb_training` WHERE `sportid`=1 AND YEAR(FROM_UNIXTIME(`time`))='.$Year.' GROUP BY (`typid` = '.WK_TYPID.'), MONTH(FROM_UNIXTIME(`time`))');
	foreach ($Data as $dat) {
		if ($dat['wk'] == 1)
