...
var minimumWindowSize = 1000;

$(window).resize(function() { jCheckForAjaxWindowSize(); })
...
function jCheckForAjaxWindowSize() {
	if ($("body").width() == minimumWindowSize) $("#ajax").addClass('tooBig');
	else $("#ajax").removeClass('tooBig');
}
...
function jWindow() {
	$("a.window").unbind("click").click(function(){
		...
		$("#ajax").css({'margin-top':($(window).scrollTop()+10)+'px'});

		if (rel == "small") {
			$("#ajax").removeClass('bigWin').addClass('smallWin');
		} else if (rel == "big") {
			$("#ajax").removeClass('smallWin').addClass('bigWin');
		}
...
function jSubmit() {
	$("form.ajax").unbind("submit").submit(function(e){
		...
		if (form_id == "search" && $("form.ajax input[name=send_to_multiEditor]:checked").length == 0) {
		...
		if (form_id != "upload" && form_id != "training" && form_id != "search" && form_id != "tcxUpload" && form_id != "newtraining" && form_id != "sportler" && form_id != "config" && form_id != "pluginconfig" && form_id != "schuhe" && form_id != "schuhe_edit") {
...
function jTraining() {
	$("#daten tr.training, a.training").unbind("click").click(function() {
		...
		$("ul.tabs li").removeClass("active");
		$("#tab_content").fadeTo(fadeSpeed, 0.01, function(){
			if ($("#tab_content_prev").attr('title') != id) {
				$("ul.tabs li#tabs_back").show().addClass("active");
				$("#tab_content_prev").html( $("#tab_content").html() ).attr('title', id);
			} else {
				$("ul.tabs li#tabs_back").hide();
				$("#tab_content_prev").html('');
			}
...
function jTablesorter() {
	$("table.sortable.autosort").tablesorter({
...
function jTablesorterWithPagination() {
	$("table.sortable.pager").tablesorter({
		textExtraction: jTablesorterTextExtraction(),
		widgets:['zebra'],
