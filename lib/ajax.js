...
function closeOverlay() {
	...
	$("#ajax, #overlay").fadeTo(400,0,function(){
		$("#overlay, #ajax").hide();
		$("#ajax").html('').css({'width':'800px','margin-left':'-400px'});
		$("#wait").fadeOut();
	});
}
...
function jSubmit() {
	$("form.ajax").unbind("submit").submit(function(e){
		...
		if (form_id != "sportler" && form_id != "config" && form_id != "schuhe" && form_id != "schuhe_edit") {
...
function jTraining() {
	$("#daten tr.training, a.training").unbind("click").click(function() {
		...
		var href = 'lib/stats/training.php?id='+id;
		if ($(this).is("a"))
			href = $(this).attr("href");
...
function seite(file,id) {
	wait(1);
	nocache = Math.random();
	http.open('get', 'lib/'+file+'.php?id='+id+'&nocache = '+nocache);
...
function seiteReply() {
	if (http.readyState == 4) {
		...
		e = document.getElementById('info_results');
		e.innerHTML = http.responseText;
		e.style.display = 'block';
		ready();
	}
...
function diagramm(file,jahr) {
	wait(1);
	http.open('get', 'lib/diagramm_'+file+'.php?jahr='+jahr+'&nocache='+Math.random());
	http.onreadystatechange = diagrammReply;
	http.send(null);
}

function diagrammReply() {
	if (http.readyState == 4) {
		wait(0);
		document.getElementById('info').style.display = 'block';
		document.getElementById('info_results').style.display = 'none';
		e = document.getElementById('info_diagramm');
		e.innerHTML = http.responseText;
		e.style.display = 'block';
		ready();
	}
...
function submit_config(id) {
	...
	for (var i=0; i<f.length; i++) {
		...
		if (j.type == "checkbox") {
			...
			if (j.checked == true) sendDaten += f[i].name+'='+encodeURIComponent(f[i].value);
		}
		else sendDaten += f[i].name+'='+encodeURIComponent(f[i].value);
		if ( i < f.length-1 ) sendDaten += '&';
	}
	nocache = Math.random();
	http.open('POST', 'lib/config.php?submit=true&id='+id+'&nocache = '+nocache);
	http.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	http.setRequestHeader("Content-length", sendDaten.length);
	http.setRequestHeader("Connection", "close");
	http.send(sendDaten);
	http.onreadystatechange = trainingReply;
}

function refreshSports(start, ende) {
	wait(1);
	http.open('get', 'lib/panel_sports_single.php?start='+start+'&ende='+ende+'&nocache='+Math.random());
	http.onreadystatechange = refreshReplySports;
	http.send(null);
}

function refreshReplySports() {
	if (http.readyState == 4) {
		wait(0);
		e = document.getElementById('sports');
		e.innerHTML = http.responseText;
		e.style.display = "block";
	}
}
...
function formular() {
	wait(1);
	http.open('get', 'lib/formular.php');
	http.onreadystatechange = formularReply;
	http.send(null);
}

function formularReply() {
	if (http.readyState == 4) {
		wait(0);
		document.getElementById('info').style.display = 'block';
		e = document.getElementById('formular');
		e.innerHTML = http.responseText;
		e.style.display = 'block';
		ready();
	}
...
function stat(file,jahr,sport,dat) {
	wait(1);
	http.open('get', 'lib/stats/'+file+'.php?jahr='+jahr+'&sport='+sport+'&dat='+dat+'&nocache='+Math.random());
	http.onreadystatechange = statReply;
	http.send(null);
}
...
function statReply() {
	if (http.readyState == 4) {
		wait(0);
		e = document.getElementById('stat-content'); 
		e.innerHTML = http.responseText;
		e.style.display="block";
		ready();
	}
...
function ajax_close() {
	document.getElementById('info_results').style.display = 'none';
	document.getElementById('info_diagramm').style.display = 'none';
	document.getElementById('formular').style.display = 'none';
	document.getElementById('info').style.display = 'none';
}

function training_tr(id) {
	element_id = 'training-' + id;
	s = document.getElementById(element_id).style.display;
	if (s == 'table-row') document.getElementById(element_id).style.display = 'none';
	else document.getElementById(element_id).style.display = 'table-row';
}

function clap(id) {
	wait(1);
	nocache = Math.random();
	http.open('get', 'lib/clap.php?id='+id+'&nocache = '+nocache);
	http.onreadystatechange = clapReply;
	http.send(null);
	element_id = 'panel-' + id;
	s = document.getElementById(element_id).style.display;
	if (s == 'none') document.getElementById(element_id).style.display = 'block';
