...
function jCheckForAjaxWindowSize() {
	if ($("#ajax").height() > $(window).height() || $("#ajax").width() > $(window).width()) {
		$("#ajax").addClass('tooBig');
		$("#ajax").css('width', (0.9*$(window).width()));
		$("#ajax").css('height', (0.9*$(window).height()));
		$("#ajax").css('margin-left', (-0.45*$(window).width()));
	}
...
function jReloadContent() {
	...
	$("#r div.panel").each(function(){
		var id = $(this).attr('id');
		jLoadLink(id, 'inc/class.Plugin.display.php?id='+id.substring(6), null)
	})
}
