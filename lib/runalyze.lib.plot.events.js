			...
			if (opt.series.points.show) {
				if (item) {
					...
					content = content + line(
						options.atString,
						axes.xaxis.tickFormatter(Math.round(pos.x*100)/100, axes.xaxis)
					) + line(
						item.series.label,
						item.series.yaxis.tickFormatter(Math.round(pos.y*100)/100, item.series.yaxis)
					);
				}
				...
				if (coords.x >= axes.xaxis.min && coords.x <= axes.xaxis.max && coords.y >= axes.yaxis.min && coords.y <= axes.yaxis.max) {
					...
					content = content + line(
						options.atString,
						axes.xaxis.tickFormatter(Math.round(coords.x*100)/100, axes.xaxis)
					);
