	...
	var RunalyzePlot = {
			...
			save: function(idOfPlot) {
				var obj = this.getPlot(idOfPlot),
					plot = $("#"+idOfPlot+" canvas.base")[0],
					copy = document.createElement('canvas');

				this.initSaver();

				if (plot.getContext) {
					var plotx = plot.getContext('2d'),
						copyx = copy.getContext('2d'),
						bg = new Image();

					obj.getOptions().grid.canvasText.show = true;
					obj.getOptions().grid.color = "#FFFFFF";
					RunalyzePlot.redraw(idOfPlot);

					copy.width = plot.width;
					copy.height = plot.height;
					copyx.width = plot.width;
					copyx.height = plot.height;
					copyx.drawImage(plot, 0, 0);
					plotx.clearRect(0, 0, plot.width, plot.height);

					bg.src = '/runalyze/img/dash.png';
					bg.onload = function() {
						var pattern = plotx.createPattern(bg, 'repeat');
						plotx.fillStyle = '#60A920';
						plotx.fillRect(0, 0, plot.width, plot.height);
						plotx.fillStyle = pattern;
						plotx.fillRect(0, 0, plot.width, plot.height);

						plotx.drawImage(bg, 0, 0);
						plotx.drawImage(copy, 2, 0);

						var imageurl = plot.toDataURL("image/png");
						$("#savePng_image").val(imageurl);
						$("#savePng").submit();

						obj.getOptions().grid.canvasText.show = false;
						obj.getOptions().grid.color = "#545454";
						RunalyzePlot.redraw(idOfPlot);
					}
				}
			},
