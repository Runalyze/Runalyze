...
(function(){
	var RunalyzePlot = {
			...
			addAnnotationTo: function(idOfPlot, x, y, text) {
				var o = this.getPlot(idOfPlot).pointOffset({'x':x, 'y':y});
				$("#"+idOfPlot).append('<div class="annotation" style="left:'+(o.left)+'px;top:'+o.top+'px;">'+text+'</div>');
			...
			initTrainingNavitation: function() {
				$("#checkForMultiplePlots").off('change').on('change', function(){
					if ($(this).is(':checked')) {
						...
						$('#plotNavigation .active:not(:first)').trigger('click');
			...
			toggleTrainingChart: function(key) {
				var $active = $("#plotNavigation div.plotToggler.active"),
					$element = $('#toggle-'+key),
