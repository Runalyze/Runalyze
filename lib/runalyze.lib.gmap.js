	...
	var RunalyzeGMap = {
			...
			init: function(id) {
				if (googleIsThere()) {
					...
					$(id).append('<p class="error">GoogleMaps ist derzeit nicht verf&uuml;gbar.</p>').css({height:'auto'});
				}
							...
							events: {
								mouseover: function(marker, event, data) {
									...
									$(RunalyzeGMap.id).gmap3({
										...
										offset: { y: -20, x: 10},
										content: '<div id="mapOverlay" class="map-tooltip">'+data+'</div>'
									});
								},
								mouseout: function() {
									$("#mapOverlay").remove();
					...
					var eventsOption = {
							...
							mousemove: function(path, position) {
								...
								if (!RunalyzeGMap.mouseIsOverPolyline) {
									$("#mapOverlay").remove();
									$(RunalyzeGMap.id).append('<div id="mapOverlay" class="map-tooltip withLeftArrow" />');
								}
			...
			resize: function() {
				if (googleIsThere() && $(RunalyzeGMap.id).gmap3('get') != null && $(RunalyzeGMap.id).css('opacity') > 0) {
