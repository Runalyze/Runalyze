	...
	var RunalyzeGMap = {
			...
			resize: function() {
				if (googleIsThere() && $(RunalyzeGMap.id).css('opacity') > 0) {
					if ($("#training-map").hasClass('fullscreen'))
						$(this.id).height( $(RunalyzeGMap.id).height() + $("#training-map").height() - $("#training-map .toolbar-box-content").outerHeight() );
					else
						$(this.id).height('');
			...
			fullScreen: function() {
				$("#training-map").addClass('fullscreen');
				RunalyzeGMap.resize();
				$("#map-fullscreen-link").text('Vollbild verlassen');
			},

			exitFullScreen: function() {
				$("#training-map").removeClass('fullscreen');
				RunalyzeGMap.resize();
