Runalyze.ActivityForm = (function($, Common){
    ...
    function isWeatherEmpty() {
        var isConditionUnknown = '1' === weather.find('#activity_weatherid').val();
    ...
    function initialUpdates() {
        ...
        updateVisibilitiesForSport(form.find('#activity_sport').find(':selected:first'));

        updatePace();

        if (!(parseInt(form.find('input[name=activity_kcal]').val()) > 0)) {
            updateCalories();
        }
    ...
    function initChangeSportHandler() {
        form.find('#activity_sport').change(function() {
    ...
    function initChangeRaceHandler() {
        form.find('#activity_is_race').change(function() {
            form.find('.only-race').toggle(out !== false);
    ...
    function initChangeDistanceOrDurationHandler() {
        form.find('input[name=activity[distance]], input[name=activity[s]]').change(function() {
            updatePace();
    ...
    function updateVisibilitiesForSport(elem) {
        var sportId = elem.val(),
            elemType = form.find('#activity_type'),
            kcal = elem.attr('data-kcal'),
            internal = elem.attr('data-internal'),
            out = elem.attr('data-outside'),
            dis = elem.attr('data-distances'),
            pow = elem.attr('data-power'),
            equipmentTypeRelation = elem.attr('data-equipment-type');

        if (kcal > 0)
            caloriesPerHour = kcal;

        form.find('.only-running').toggle(typeof internal !== 'undefined' && internal !== false && internal === 1);
        form.find('.only-not-running').toggle(typeof internal === 'undefined' || internal !== 1);
    ...
    function updateWeatherDataFrom(data) {
        if (data && !data.empty) {
            ...
            weather.find('#activity_temperature').val(Math.round(data.temperature));
            weather.find('#activity_wind_speed').val(Math.round(data.wind_speed));
            weather.find('#activity_wind_deg').val(data.wind_deg);
            weather.find('#activity_humidity').val(data.humidity);
            weather.find('#activity_pressure').val(data.pressure);
    ...
    function tryToLoadWeatherData(hideIfRequestFails) {
        ...
        var date = Common.parseDate($("#activity_time_day").val(), 'dd.mm.yyyy');
    ...
    function hideAndUnsetWeatherInputs() {
        ...
        weather.find('#activity_weatherid').val('1');
