Runalyze.ActivityForm = (function($, Common){
    ...
    function initialUpdates() {
        ...
        if (isWeatherEmpty()) {
            if (isNewActivity() && options.loadWeather) {
    ...
    function updateWeatherDataFrom(data) {
        if (data && !data.empty) {
            weather.find('#weatherid option[value=' + data.weatherid + ']').attr('selected', true);
            ...
            weather.find('.weatherdata-source').html('via ' + data.source.name);
    ...
    function hideAndUnsetWeatherInputs() {
        ...
        weather.find('#weatherid option[value=1]').attr('selected', true);
