...
(function(){
	var Runalyze = {
			options: {
				...
				urlForPanels: 'call/call.ContentPanels.php',
			...
			initForReload: function() {
				var currentTab = this.currentTabContentUrl;
				...
				this.currentTabContentUrl = currentTab;
				this.tabs.removeClass("active");
				this.tabs.children("a[href='"+Runalyze.currentTabContentUrl+"']").parent().addClass("active");
			},
			...
			initOverlay: function() {
				if ($("#container, #statistics-inner, #ajax").length == 0) {
					...
					this.body.prepend('<div id="overlay"></div><div id="ajax-outer"><div id="ajax" class="panel"></div></div>');					
				}

				this.overlay    = $("#overlay");
				this.ajax       = $("#ajax");

				$("#ajax-outer").click(function(e){
					if (e.target == e.currentTarget)
						Runalyze.closeOverlay();
				});
			},
			...
			initTooltip: function() {
				...
				$("body > .tooltip").remove(); // Remove old tooltips
			...
			initFormulars: function() {
				...
				$("#ajax form.ajax").unbind("submit").submit(function(e){
					...
					// Remove old datepicker from node tree
			...
			closeOverlay: function() {
				if ($("#container, #data-browser, #statistics-inner").length > 0)
					$("#ajax, #overlay, #ajax-navigation").fadeTo(this.options.fadeSpeed, 0, function(){
						...
						Runalyze.body.removeClass('overlay');
					});
			},
			...
			loadTraining: function(id, sharedUrl) {
				var url = this.options.urlForTraining+'?id='+id;
			...
			reloadContent: function() {
				var url = '';
				if (this.currentTabContentUrl.indexOf(this.options.urlForTraining) >= 0) {
					var id = this.currentTabContentUrl.substr(this.options.urlForTraining.length + 4);
					url = "index.php?id="+id+" #container > *";
				} else if (this.currentTabContentUrl.indexOf(this.options.urlForStat) >= 0) {
					var id = this.currentTabContentUrl.substr(this.options.urlForStat.length);
					url = "index.php?pluginid="+id+" #container > *";
				}

				if (url.length > 0) {
					var e = $("#container").addClass('loading');
			...
			reloadTraining: function() {
				if (this.currentTabContentUrl.substr(0,30) == 'call/call.Training.display.php')
					this.reloadCurrentTab();
			},
			...
			reloadAllPanels: function(dontclass) {
				if (typeof dontclass == "undefined") {
					$("#panels").loadDiv( this.options.urlForPanels );
				} else {
