...
function bindFlotForQTipBars(elem, plot) {
	...
	elem.bind('plothover', function(event, coords, item) {
        ...
        if (item) {
			...
			content = "<em>"+item.series.yaxis.tickFormatter(Math.round(y*100)/100, item.series.yaxis)+"</em>";
...
function bindFlotForSelection(elem, plot) {
	...
	elem.bind('plotselected', function(event, ranges) {
		...
		var axes = plot.getAxes(),
			...
			o = plot.pointOffset({x:to, y:(to-from)/2});

		content = content + tipLineFor(axes.xaxis.tickFormatter(from)+" - "+axes.xaxis.tickFormatter(to), axes.xaxis.tickFormatter(Math.round((to-from)*10)/10));

		// TODO: Think if min/max value is of interes too
		var i, j, dataset = plot.getData(), num = 0, sum = 0;
		for (i = 0; i < dataset.length; ++i) {
			var series = dataset[i];
		...
		$("#"+$(this).attr('id')+" .hoverTip").show().html(content).css('top',elem.offset.top+o.top).css('left',elem.offset.left+o.left);
	});
